...
# Checking that the minimum wind speed is at least 0 (without NA)
Observations_cleaned_data$`Wind speed in km/h` |> min(na.rm = TRUE) >= 0

# Checking that the minimum waterfowl population is at least 0 (without NA)
Observations_cleaned_data$`Waterfowl population` |> min(na.rm = TRUE) >= 0

# Checking that the minimum turbidity is at least 0 (without NA)
Observations_cleaned_data$`Turbidity of water (NTUs)` |> min(na.rm = TRUE) >= 0

# Checking the range of the wind speed (without NA)
Observations_cleaned_data$`Wind speed in km/h` |>
  range(na.rm = TRUE)

# Checking the range of the waterfowl population (without NA)
Observations_cleaned_data$`Waterfowl population` |>
  range(na.rm = TRUE)

# Checking the range of the water turbidity (without NA)
